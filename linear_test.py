...
geneLsts = {}   # store the top M genes from scoring system
M = 50 # Top M genes in each category
...
#    XX=X.apply(lambda row: row*temp,axis=1)
    ...
    topIndices = [x[0] for x in sorted(enumerate(scores), key=lambda e:e[1])][:M]
    #geneLsts[name] = XX.ix[topIndices].index.tolist()
    geneLsts[name] = young.labels[young.labels[name]==1].index.tolist()
